...
static tree_predict predict_func_merge[4] = {
...
*/
double INLINE vect_average_blck_int8(const uint8_t* _mat_src, size_t _x, size_t _x_end, size_t _y, size_t _y_end, size_t _width)
{
	if (_mat_src == NULL)
	{
...
* \return average value of the block, -1 if error.
*/
double INLINE vect_variance_blck_int8(const uint8_t* _mat_src, size_t _x, size_t _x_end, size_t _y, size_t _y_end, double _avg_blck, size_t _width)
{
	if (_mat_src == NULL)
	{
		fprintf(stderr, "null pointer as parameter.");
