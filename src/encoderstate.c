...
static void encoder_state_write_aud(encoder_state * const encoder_state)
{
...
static void worker_sao_reconstruct_lcu(void *opaque) {
...
static void encoder_state_write_bitstream_main(encoder_state * const main_state) {
  ...
  if (main_state->global->is_radl_frame) {
    ...
    if (encoder->aud_enable)
      encoder_state_write_aud(main_state);
