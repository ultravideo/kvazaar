                                ...
                                uint32_t *inter_bitcost)
{
  ...
  bool is_ILR = info->state->frame->ref->image_info[info->ref_idx].is_long_term && info->state->encoder_control->layer.layer_id > info->state->frame->ref->image_info[info->ref_idx].layer_id;
  ...
  if (!is_ILR && cfg->fme_level > 0 && info->best_cost < *inter_cost) {
    search_frac(info);
    ...
    for (int32_t idx = 0; idx < num_cand_pairs; idx++) {
      ...
      if ((merge_cand[i].dir & 0x1) && (merge_cand[j].dir & 0x2)) {
        if (merge_cand[i].ref[0] != merge_cand[j].ref[1] ||
          ...
          if (state->frame->refmap[merge_cand[i].ref[0]].list == 2 ||
              state->frame->refmap[merge_cand[j].ref[1]].list == 1) continue;
